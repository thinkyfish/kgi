...
static kii_u_t event_device_unit(struct cdev *device, struct thread *td)
{
	kii_u_t unit = -1;
	...
	if (minor(device))
		...
		if (td)
			unit = minor(td->td_proc->p_pgrp->pg_session-> \
				     s_ttyp->t_dev);
		else
			unit = 0;
...
**	command resource
*/
static int event_command(event_file_t *file, unsigned int cmd, 
...
/*	ioctl() services.
*/
static int event_ioctl(struct cdev *dev, u_long cmd, caddr_t data, int flags,
		       struct thread *td)
{
	kii_u_t unit;
	event_file_t *file;
...
*/
static int event_read(struct cdev *dev, struct uio *uio, int ioflag)
{
	event_file_t *file;
	int error, count;
...
static int event_poll(struct cdev *dev, int poll, struct thread *td)
{
	event_file_t *file;
...
static int event_open(struct cdev *dev, int flag, int mode, struct thread *td)
{
	event_file_t *file;
	void *queue_buffer;

	KRN_DEBUG(1, "open() /dev/event%i", minor(dev));
	...
	if (!(file = event_files[minor(dev) - 1])) {
...
static int event_release(struct cdev *dev, int flags, int mode, struct thread *td)
{
	kii_u_t unit;
	event_file_t *file;
...
static int dev_event_init(void)
{
