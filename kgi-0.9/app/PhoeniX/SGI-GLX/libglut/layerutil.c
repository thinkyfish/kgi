...
{
  unsigned int i, screen = vinfo->screen;
  ...
  XLayerVisualInfo * lvinfo_template, int *nitems_return)
{
  ...
  unsigned int numVisuals, count, i, j;
  ...
  for (i = 0; i < numVisuals; i++) {
	...
	if (overlayInfo == NULL) {
		...
		if (lvinfo_template->layer != overlayInfo->layer) {
	...
	if (overlayInfo == NULL) {
		...
		if (lvinfo_template->type != overlayInfo->transparent_type) {
	...
	if (lvinfo_template->value != overlayInfo->value) {
	        continue;
	}
    }
