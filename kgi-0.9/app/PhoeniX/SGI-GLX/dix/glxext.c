...
**
** $Date$ $Revision$
** $Header: //depot/main/glx/server/glxext.c#12 $
*/
...
Bool GlxInitVisuals(VisualPtr *visualp, DepthPtr *depthp, int *nvisualp,
	...
	unsigned long sizes, int bitsPerRGB, int preferredVis)
{
	...
	if (saveInitVisualsProc) {
		...
		ret = saveInitVisualsProc(visualp, depthp, nvisualp,
			ndepthp, rootDepthp, defaultVisp,
			sizes, bitsPerRGB, preferredVis);
...
#if 0
	(*__glXExt->initVisuals)(visualp, depthp, nvisualp, ndepthp,
		rootDepthp, defaultVisp, sizes, bitsPerRGB);
#endif
	return True;
}

void GlxWrapInitVisuals(miInitVisualsProcPtr *initVisProc)
{
