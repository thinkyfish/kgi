...
#if 0
static inline void VIRGE_EDAC_OUT8(virge_clock_io_t *virge_io, 
...
#warning save initial clock settings here! -- seeger_s

	return KGI_EOK;
}

void virge_clock_done(virge_clock_t *virge, virge_clock_io_t *virge_io,
	const kgim_options_t *options)
{
#warning restore initial clock settings here! -- seeger_s

	KRN_DEBUG(2, "virge_clock_done()");
}

kgi_resource_t *virge_clock_mode_resource(virge_clock_t *virge,
	virge_clock_mode_t *virge_mode, kgi_image_mode_t *img,
	kgi_u_t images, kgi_u_t index)
{
	...
	if (img->fam & KGI_AM_TEXTURE_INDEX) {
		...
		return vga_clock_mode_resource(&virge->vga,
			&virge_mode->vga, img, images, index);
	}
#endif
	/*	We don't handle any other clock resources yet.
	*/
	return NULL;
}

void virge_clock_mode_prepare(virge_clock_t *virge, virge_clock_io_t *virge_io,
	virge_clock_mode_t *virge_mode, kgi_image_mode_t *img, kgi_u_t images)
{
	KRN_DEBUG(2, "virge_clock_mode_prepare()");
#warning if you do not need this, do not implement it. -- seeger_s
}

void virge_clock_mode_enter(virge_clock_t *virge, virge_clock_io_t *virge_io, 
	virge_clock_mode_t *virge_mode, kgi_image_mode_t *img, kgi_u_t images)
{
	kgi_u_t cnt;
	...
	KRN_ASSERT(virge_mode->pll_mode.mul < 256);
	KRN_ASSERT(virge_mode->pll_mode.div < 256);
	KRN_ASSERT(virge_mode->pll_mode.p   < 8);
	
	KRN_DEBUG(2, "mul = %d, div = %d, p = %d", mult, div, p);

#warning driver does not set clock yet! -- seeger_s
#if 0
	/*	We use clock C, as this one is 'not valid' at boot up
	*/
	VIRGE_EDAC_OUT8(virge_io,
		(virge_mode->pll.mul) & 0xFF, VIRGE_EDAC_PClkC_M);
	VIRGE_EDAC_OUT8(virge_io,
		(virge_mode->pll.div) & 0xFF, VIRGE_EDAC_PClkC_N); 
	VIRGE_EDAC_OUT8(virge_io,
		((virge_mode->pll.p) & 0x07) | VIRGE_EDAC_PClkP_Enable,
		VIRGE_EDAC_PClkC_P);
	...
	cnt = 10000;
	while (cnt-- &&
		(VIRGE_DAC_IN8(virge_io, VIRGE_DAC_EXT_DATA) & 
		VIRGE_EDAC29_PLLlocked)) {
	}
...
void virge_clock_mode_leave(virge_clock_t *virge, virge_clock_io_t *virge_io,
	virge_clock_mode_t *virge_mode, kgi_image_mode_t *img, kgi_u_t images)
{
