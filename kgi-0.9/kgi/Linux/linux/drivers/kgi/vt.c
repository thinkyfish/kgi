...
#define	IOCTL(x)	printk(KERN_ERR "%s: " x "\n", current->comm)
...
#warning	check SAK, etc.
	...
	*/
	case VT_ACTIVATE:
		...
		*/
		KRN_DEBUG(2, "activating switch to %i", arg - 1);
		{
			int console = 0;
	...
	*/
	case VT_RELDISP:
		if (cons->vt_mode.mode != VT_PROCESS) {
			...
			KRN_DEBUG(1, "VT_RELDISP: console not in PROCESS mode");
			return -EINVAL;
		}
		switch (arg) {
		...
		case 0:
			if (cons->vt_ack == VT_ACK_PENDING_FROM) {
				...
				cons->vt_ack = VT_ACK_IDLE;
				f->want_console = KII_INVALID_CONSOLE;
				KRN_DEBUG(3, "pending from disallowed");
				return EOK;
			}
			break;
		...
		case 1:
			...
			KRN_DEBUG(1, "invalid pending-from-ack");
			break;
		...
		case 2:
			...
			KRN_DEBUG(1, "invalid pendig-to-ack");
			break;
		}
		return -EINVAL;
	...
	case VT_PROCESS:
		...
		*/
		...
		case VT_ACK_IDLE:
			...
			*/
			KRN_DEBUG(3, "process died? resetting console");
			vt_reset_vc(cons);
			break;
